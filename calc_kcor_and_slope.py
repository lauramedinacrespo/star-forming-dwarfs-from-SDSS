...
def get_kcor_and_slope(filter_name, redshift, colour_name, colour_value):
    """
    Módulo adaptado para el cálculo de correcciones K.
    Basado en el código original de:
    Copyright 2009-2013 Chilingarian, I., Melchior. A.-L., and Zolotukhin, I.
    
    > Modificaciones realizadas por Laura Medina Crespo (2026):
    > - Calcular la pendiente (derivada) de la K-correction para propagación de errores.
    ...
    # Clave de búsqueda (ej: 'g_gr')
    key = filter_name + '_' + colour_name.replace(' - ', '').replace('-', '')
    
    if key not in coeff:
        raise ValueError(f"Combinación {key} no encontrada en los coeficientes.")

    c = coeff[key]
    
    # Inicializamos arrays (asumiendo que redshift y colour_value son numpy arrays)
    # Si son escalares, numpy lo maneja bien igual.
    k_val = np.zeros_like(redshift)
    k_slope = np.zeros_like(redshift) # Esto será dK/dColor

    # El doble bucle original de Chilingarian, pero VECTORIZADO para arrays
    # x = potencia de redshift, y = potencia de color
    for x, row in enumerate(c):
        for y, val in enumerate(row):
            if val == 0: continue # Optimización pequeña
            
            # 1. Acumulamos el valor de K
            # Term = Coeff * z^x * color^y
            k_val += val * (redshift**x) * (colour_value**y)
            
            # 2. Acumulamos la pendiente (derivada respecto al color)
            # Derivada de (color^y) es y * color^(y-1)
            if y > 0:
                k_slope += val * (redshift**x) * (y * colour_value**(y-1))
                
    return k_val, np.abs(k_slope)